...
mysql_select_db($db_name);
$FindEmail = mysql_query("SELECT email FROM `user_account` WHERE guid = '$guid'")or die(" : " . mysql_error());
$result = mysql_fetch_array($FindEmail); //query translate to correct value;
$email = $result[0];
//
$data =mysql_query("SELECT * FROM `sold_book` where userID = '$email' AND state = 0");
$page = 3;
